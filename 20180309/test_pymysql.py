...
# 创建连接
conn = pymysql.connect(host='127.0.0.1', port=3306, user='test_user', passwd='test_user', db='canxue_55',charset='utf8')
# 创建连接的游标
cursor = conn.cursor()  #默认返回元组，一个记录时就是单个元组，多个记录时就是元组列表
cursor = conn.cursor(cursor=pymysql.cursors.DictCursor) #将遍历结果由元组改为字典

# 执行SQL，并返回收影响行数
effect_row = cursor.execute("select * from dim_node")
...
cursor.scroll(1,mode='absolute') # 相对绝对位置移动，从头开始，此时不能为负
print set_row ,'\n', cursor.fetchmany(3)
